...
1. `blank` trong v1.0 chỉ hỗ trợ **một chỗ trống**. Nhiều chỗ trống là phạm vi của v1.1 trở lên.
...
| `hint` | Không | string | Gợi ý cho chế độ Luyện tập hoặc Học tập |
...
- `question_text` phải chứa đúng **một** placeholder chính thức:
...
question_text có nhiều hơn một [[blank]]
...
1. `question_text` phải chứa đúng một `[[blank]]`
...
## 12. Hành vi UI tối thiểu cho Import Preview
...
5. Nút `Import` chỉ bật khi không có `ERROR`
6. Nút `Cancel` luôn khả dụng

### 12.2. Những gì không được làm

1. Không import ngay khi người dùng vừa chọn file mà không preview
2. Không nuốt lỗi thầm lặng
3. Không chỉ hiển thị “Import failed” chung chung mà không có line-level detail
...
1. Import thành công cho từng loại câu hỏi: `multiple_choice`, `multiple_answer`, `blank`, `short_answer`
...
7. Fail khi `blank` có nhiều hơn 1 `[[blank]]`
...
1. Import file hợp lệ và kiểm tra dữ liệu thực sự đã được lưu DB
2. Import file có lỗi và kiểm tra DB không bị ghi một phần

...
1. Trong nhóm **Xuất đề thi (.docx)**, chọn loại đề:
   - `Trắc nghiệm`
   - `CRQ`
   - `Hỗn hợp`
2. Nếu chọn `CRQ` hoặc `Hỗn hợp`, ứng dụng sẽ mở cấu hình điểm cho các câu CRQ.
3. `CRQ` không sinh phiếu trả lời riêng vì phần trả lời nằm ngay sau câu hỏi.
4. `Hỗn hợp` chỉ sinh phiếu trả lời cho câu trắc nghiệm; phần CRQ vẫn nằm ở cuối đề và được đánh số lại từ `1`.
5. Có thể lưu lại preset xuất đề để dùng nhanh cho các lần sau.
...
> Ghi chú: các câu hỏi `Tự luận` và `Bài toán` đều thuộc family `CRQ` nhưng vẫn được giữ riêng để phục vụ thống kê và export.
